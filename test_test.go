...
func GenericTest(t *testing.T, part string, nclients int, unreliable bool, crash bool, partitions bool, maxraftstate int) {
	...
	for i := 0; i < 3; i++ {
		...
		DPrintf("spawn_clients_and_wait at %d", i)
		go spawn_clients_and_wait(t, cfg, nclients, func(cli int, myck *Clerk, t *testing.T) {
			...
			DPrintf("cli %d put key %s", cli, key)
			...
			for atomic.LoadInt32(&done_clients) == 0 {
				if (rand.Int() % 1000) < 500 {
					...
					DPrintf("cli %d Append key %s value %s", cli, key, nv)
					last = NextValue(last, nv)
					j++
				} else {
					// log.Printf("%d: client new get %v\n", cli, key)
					v := Get(cfg, myck, key)
					DPrintf("cli %d Get key %s value %s", cli, key, v)
					if v != last {
